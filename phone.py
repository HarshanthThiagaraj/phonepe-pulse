...
st.subheader("THE BEAT OF PROGRESS")



path=r"C:\Users\harsh\Documents\phonepe\pulse\data\aggregated\transaction\country\india\state/"
Agg_state_list=os.listdir(path)

#state wise aggregated transactions
...
#state wise aggregated users
path2 = r"C:\Users\harsh\Documents\phonepe\pulse\data\aggregated\user\country\india\state/"
...
#state wise map transactions
path3 = r"C:\Users\harsh\Documents\phonepe\pulse\data\map\transaction\hover\country\india\state/"
...
#state wise map users
path4 =  r"C:\Users\harsh\Documents\phonepe\pulse\data\map\user\hover\country\india\state/"
...
#state wise top transactions
path5 = r"C:\Users\harsh\Documents\phonepe\pulse\data\top\transaction\country\india\state/"
...
#state wise top users
path6 = r"C:\Users\harsh\Documents\phonepe\pulse\data\top\user\country\india\state/"
...
#India aggregated transactions
path7=r"C:\Users\harsh\Documents\phonepe\pulse\data\aggregated\transaction\country\india/"
tyear_list=os.listdir(path7)
tyear_list.pop()

col7={'Year':[],'Quarter':[],'Transaction_type':[], 'Transaction_count':[], 'Transaction_amount':[]}
for j in tyear_list:
        p_j=path7+j+"/"
        Agg_yr_list=os.listdir(p_j)
        for k in Agg_yr_list:
            ...
            for z in G['data']['transactionData']:
              ...
              count=z['paymentInstruments'][0]['count']
              amount=z['paymentInstruments'][0]['amount']
              col7['Transaction_type'].append(Name)
              col7['Transaction_count'].append(count)
              col7['Transaction_amount'].append(amount)
              col7['Year'].append(j)
              col7['Quarter'].append(int(k.strip('.json')))
df_aggregated_indtransaction=pd.DataFrame(col7)

#India aggregated users
path8=r"C:\Users\harsh\Documents\phonepe\pulse\data\aggregated\user\country\india/"
uyear_list=os.listdir(path7)
uyear_list.pop()

col8={'Year':[],'Quarter':[],'brands':[], 'Count':[], 'Percentage':[]}
for j in tyear_list:
        p_j=path8+j+"/"
        Agg_yr_list=os.listdir(p_j)
        for k in Agg_yr_list:
            ...
            H=json.load(Data)
            try:
                for w in H["data"]["usersByDevice"]:
                    brand_name = w["brand"]
                    count_ = w["count"]
                    ALL_percentage = w["percentage"]
                    col8["brands"].append(brand_name)
                    col8["Count"].append(count_)
                    col8["Percentage"].append(ALL_percentage)
                    col8["Year"].append(j)
                    col8["Quarter"].append(int(k.strip('.json')))
            except:
                pass
df_aggregated_induser = pd.DataFrame(col8)

#India map transactions
path9=r"C:\Users\harsh\Documents\phonepe\pulse\data\map\transaction\hover\country\india/"
ihover_list=os.listdir(path9)
ihover_list.pop()

col9={'Year':[],'Quarter':[],'State':[],'count':[],'amount':[]}
for j in ihover_list:
    p_j=path9+j+'/'
    Agg_yr_list=os.listdir(p_j)
    for k in Agg_yr_list:
        p_k=p_j+k
        Data=open(p_k,'r')
        I=json.load(Data)
        for x in I["data"]["hoverDataList"]:
                District = x["name"]
                count = x["metric"][0]["count"]
                amount = x["metric"][0]["amount"]
                col9["State"].append(District)
                col9["count"].append(count)
                col9["amount"].append(amount)
                col9['Year'].append(j)
                col9['Quarter'].append(int(k.strip('.json')))
df_map_indtransaction = pd.DataFrame(col9)

#India map users
path10=r"C:\Users\harsh\Documents\phonepe\pulse\data\map\user\hover\country\india/"
imap_list=os.listdir(path10)
imap_list.pop()

col10={'Year':[],'Quarter':[],'State':[],'RegisteredUser':[]}
for j in imap_list:
    p_j=path10+j+'/'
    Agg_yr_list=os.listdir(p_j)
    for k in Agg_yr_list:
        ...
        Data=open(p_k,'r')
        J=json.load(Data)
        for u in J["data"]["hoverData"].items():
                District = u[0]
                registereduser=u[1]["registeredUsers"]
                col10["State"].append(District)
                col10["RegisteredUser"].append( registereduser)
                col10['Year'].append(j)
                col10['Quarter'].append(int(k.strip('.json')))
df_map_indusers = pd.DataFrame(col10)
    
#India top transactions
path11 = r"C:\Users\harsh\Documents\phonepe\pulse\data\top\transaction\country\india/"
Top_Ilist = os.listdir(path11)
Top_Ilist.pop()

col11 = {'Year': [], 'Quarter': [], 'District': [], 'Transaction_count': [],
        'Transaction_amount': []}
for j in Top_Ilist:
        p_j = path11 + j + "/"
        Agg_yr_list = os.listdir(p_j)
        ...
        for k in Agg_yr_list:
            ...
            for z in K['data']['pincodes']:
                ...
                col5['Year'].append(j)
                col5['Quarter'].append(int(k.strip('.json')))
df_top_indtransaction = pd.DataFrame(col11)

#India top users
path12 = r"C:\Users\harsh\Documents\phonepe\pulse\data\top\user\country\india/"
User_Ilist = os.listdir(path12)
User_Ilist.pop()

col12 = {'Year': [], 'Quarter': [], 'District': [],
        'RegisteredUser': []} 
for j in User_Ilist:
        p_j = path12 + j + "/"
        Agg_yr_list = os.listdir(p_j)
        ...
        for k in Agg_yr_list:
            ...
            Data = open(p_k, 'r')
            L= json.load(Data)
            for t in L['data']['pincodes']:
                Name = t['name']
                registeredUser = t['registeredUsers']
                col12['District'].append(Name)
                col12['RegisteredUser'].append(registeredUser)
                col12['Year'].append(j)
                col12['Quarter'].append(int(k.strip('.json')))
df_top_induser = pd.DataFrame(col12)
...
#SQL connection and adding data to the database
...
df_aggregated_indtransaction.to_sql('aggregated_indtransaction', connection, if_exists='replace')
df_aggregated_induser.to_sql('aggregated_induser', connection, if_exists='replace')
df_map_indtransaction.to_sql('map_indtransaction', connection, if_exists='replace')
df_map_indusers.to_sql('map_induser', connection, if_exists='replace')
df_top_indtransaction.to_sql('top_indtransaction', connection, if_exists='replace')
df_top_induser.to_sql('top_induser', connection, if_exists='replace')


SELECT = option_menu(
        menu_title = None,
        options = ["Home","About","Insights","Search"],
        icons =["house","bar-chart","toggles","search","at"],
        default_index=0,
        orientation="horizontal",
        styles={
            "container": {"padding": "0!important", "background-color": "purple","size":"cover"},
            "icon": {"color": "black", "font-size": "20px"},
            "nav-link": {"font-size": "20px", "text-align": "center", "margin": "-2px", "--hover-color": "white"},
...
if SELECT=="Home":
    st.header("ALL INDIA")
    st.subheader("Users")
    df=pd.merge(df_map_indtransaction,df_map_indusers)
    fig1= px.bar(df,x="State",y="RegisteredUser",color="Year",title="Total Users")
    st.plotly_chart(fig1,theme=None, use_container_width=True)
    st.subheader("Transactions")
    fig2=px.bar(df,x="State",y="count",color="Year",title="Total Transactions")
...
if SELECT == "Insights":
    ...
    #st.write("----")
    st.subheader("Please select an insight")
...
if SELECT=="About":
    ...
    st.video("https://www.youtube.com/watch?v=c_1H6vivsiA&t=2s")
...
#creating functions for query search in sqlite to get the data
    ...
    def transaction_quater(_state,_year,_quater):
        cursor.execute(f"SELECT State,Year,Quarter,District,Transaction_count,Transaction_amount FROM top_transaction WHERE Year = '{_year}' AND Quarter = '{_quater}' AND State = '{_state}' GROUP BY State,Year,Quarter")
        df = pd.DataFrame(cursor.fetchall(), columns=['State', 'Year',"Quarter", 'District', 'Transaction_count', 'Transaction_amount'])
        return df
    ...
    if choice_topic=="Top-Transactions":
        ...
        with col3:
            ...
            menu_quater = ["", "1", "2", "3", "4"]
            choice_quater = st.selectbox("Quarter", menu_quater, 0)
